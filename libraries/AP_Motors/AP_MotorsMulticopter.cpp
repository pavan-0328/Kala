...
void AP_MotorsMulticopter::set_actuator_with_slew(float& actuator_output, float input)
{
    ...
    */
    ...
    if (is_positive(_slew_up_time)) {
        float output_delta_up_max = _dt_s / (constrain_float(_slew_up_time, FLT_MIN, 0.5f)); // Taking min +Ve float to avoid divide by zero 
    ...
    if (is_positive(_slew_dn_time)) {
        float output_delta_dn_max = _dt_s / (constrain_float(_slew_dn_time, FLT_MIN, 0.5f));//Taking min +Ve float to avoid divide by zero
