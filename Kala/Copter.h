...
public:
    ...
    Copter(void);
    // declare the enforcement helper
    void enforce_frame_lock();
private:
    ...
    struct {
        ...
        uint32_t start_ms = 0;  // system time high vibration were last detected
        uint32_t clear_ms = 0;  // system time high vibrations stopped
    } vibration_check;
...
# include USERHOOK_VARIABLES
    ...
    struct PACKED {
        ...
        bool pre_arm_rc_check = false;               //  3 true if rc input pre-arm checks have been completed successfully
        bool pre_arm_check = false;                  //  4 true if all pre-arm checks (rc, accel calibration, gps lock) have been performed
        bool auto_armed = false;                     //  5 stops auto missions from beginning until throttle is raised
        bool unused_log_started = false;             //  6
        bool land_complete = false;                  //  7 true if we have detected a landing
        bool new_radio_frame = false;                //  8 Set true if we have new PWM data to act on from the Radio
        bool unused_usb_connected = false;           //  9
        bool unused_receiver_present = false;        // 10
        bool compass_mot = false;                    // 11 true if we are currently performing compassmot calibration
        bool motor_test = false;                     // 12 true if we are currently performing the motors test
        bool initialised = false;                    // 13 true once the init_ardupilot function has completed.  Extended status to GCS is not sent until this completes
        bool land_complete_maybe = false;            // 14 true if we may have landed (less strict version of land_complete)
        bool throttle_zero = false;                  // 15 true if the throttle stick is at zero, debounced, determines if pilot intends shut-down when not using motor interlock
        bool system_time_set_unused = false;         // 16 true if the system time has been set from the GPS
        bool gps_glitching = false;                  // 17 true if GPS glitching is affecting navigation accuracy
        bool using_interlock = false;                // 18 aux switch motor interlock function is in use
        bool land_repo_active = false;               // 19 true if the pilot is overriding the landing position
        bool motor_interlock_switch = false;         // 20 true if pilot is requesting motor interlock enable
        bool in_arming_delay = false;                // 21 true while we are armed but waiting to spin motors
        bool initialised_params = false;             // 22 true when the all parameters have been initialised. we cannot send parameters to the GCS until this is done
        bool unused_compass_init_location = false;   // 23
        bool unused2_aux_switch_rc_override_allowed = false; // 24
        bool armed_with_airmode_switch = false;      // 25 we armed using a arming switch
    ...
    // Failsafe
    struct {
        uint32_t terrain_first_failure_ms = 0;  // the first time terrain data access failed - used to calculate the duration of the failure
        uint32_t terrain_last_failure_ms = 0;   // the most recent time terrain data access failed

        int8_t radio_counter = 0;            // number of iterations with throttle below throttle_fs_value
    ...
    struct {
        bool active = false;        // true if dead reckoning (position estimate using estimated airspeed, no position or velocity source)
        bool timeout = false;       // true if dead reckoning has timedout and EKF's position and velocity estimate should no longer be trusted
    ...
    float super_simple_cos_yaw = 0.0f;
    ...
    struct {
        bool takeoff_expected = false;
        bool touchdown_expected = false;
        uint32_t takeoff_time_ms = 0;
