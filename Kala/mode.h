...
protected:
    ...
    private:
        bool _running = false;
        float take_off_start_alt_m = 0.0f;
        float take_off_complete_alt_m = 0.0f;
    };
...
public:
    ...
    private:
        ...
        Mode _last_mode = Mode::LOOK_AT_NEXT_WP;

        // Yaw will point at this location if mode is set to Mode::ROI
        Vector3f roi_ne_m;

        // yaw used for YAW_FIXED yaw_mode
        float _fixed_yaw_offset_rad = 0.0f;

        // Radians/s we should turn
        float _fixed_yaw_slewrate_rads = 0.0f;

        // time of the last yaw update
        uint32_t _last_update_ms = 0;

        // heading when in yaw_look_ahead_yaw
        float _look_ahead_yaw_rad = 0.0f;

        // turn heading (rad) and rate (rads) when auto_yaw_mode is set to AUTO_YAW_RATE
        float _yaw_angle_rad = 0.0f;
        float _yaw_rate_rads = 0.0f;
        float _pilot_yaw_rate_rads = 0.0f;
    };
...
private:
    ...
    // Loiter control
    uint16_t loiter_time_max = 0;                // How long we should stay in Loiter Mode for mission scripting (time in seconds)
    uint32_t loiter_time = 0;                    // How long have we been loitering - The start time in millis
    ...
    uint32_t nav_delay_time_max_ms = 0;  // used for delaying the navigation commands (eg land,takeoff etc.)
    uint32_t nav_delay_time_start_ms = 0;

    // Delay Mission Scripting Command
    int32_t condition_value = 0;  // used in condition commands (eg delay, change alt, etc.)
    uint32_t condition_start = 0;
    ...
    bool waiting_to_start = false;  // true if waiting for vehicle to be armed or EKF origin before starting mission
    ...
    struct {
        int16_t roll_deg = 0;       // target roll angle in degrees.  provided by mission command
        int8_t pitch_deg = 0;       // target pitch angle in degrees.  provided by mission command
        int16_t yaw_deg = 0;        // target yaw angle in degrees.  provided by mission command
        float climb_rate_ms = 0.0f; // climb rate in m/s. provided by mission command
        uint32_t start_ms = 0;      // system time that nav attitude time command was received (used for timeout)
    } nav_attitude_time;

    // desired speeds
    struct {
        float xy = 0.0f;     // desired speed horizontally in m/s. 0 if unset
        float up = 0.0f;     // desired speed upwards in m/s. 0 if unset
        float down = 0.0f;   // desired speed downwards in m/s. 0 if unset
    } desired_speed_override_ms;

    float circle_last_num_complete = 0.0f;
};
...
private:
    ...
    FlipState _state = FlipState::Start;   // current state of flip
    Mode::Number  orig_control_mode = Mode::Number::STABILIZE;    // flight mode when flip was initiated
    uint32_t start_time_ms = 0;             // time since flip began
    int8_t roll_dir = 0;                    // roll direction (-1 = roll left, 1 = roll right)
    int8_t pitch_dir = 0;                   // pitch direction (-1 = pitch forward, 1 = pitch back)
};
...
private:
    ...
    bool nextmode_attempted = false;
    uint32_t free_fall_start_ms = 0;    // system time free fall was detected
    float free_fall_start_vel_u_ms = 0.0f;     // vertical velocity when free fall was detected
};
...
private:
    ...
    float motors_output = 0.0f;
    Vector2f motors_input;
    uint32_t last_throttle_warning_output_ms = 0;
