...
// enforce locked frame parameters (Hexa-X only)
void Copter::enforce_frame_lock()
{
...
{
    // keep FRAME_CLASS / FRAME_TYPE locked to Hexa-X
    enforce_frame_lock();
#if HAL_LOGGING_ENABLED
    if (should_log(MASK_LOG_ANY)) {
    ...
    if (!motors->armed()) {
        ...
        /// Always lock to Hexa-X, ignore FRAME_CLASS / FRAME_TYPE params
