...
#-------------------------------------------------------------------
    ...
    def testScripting_ADSB(self):
        """Testing Scripting test ADSB"""
        self.context_push()
        ...
        self.reboot_sitl()
        self.install_example_script_context("vel_stream.lua")
        self.reboot_sitl()
        self.wait_ready_to_arm()
        self.change_mode("AVOID_ADSB")
        ...
        self.change_mode("LAND")
        self.wait_landed_and_disarmed(min_alt=2,timeout=35)
        self.progress("TestedScripting test ADSB")
    ...
    def test_avoid_adsb_rcry_cd(self):
        """Testing ADSB AVOID via RCY CLIB/Decent """
        self.progress("Starting ADSB AVOID via RCY test CLIMB/DESCENT")
        self.set_parameters({"AVD_ENABLE": 1})
        self.reboot_sitl()
        self.wait_ready_to_arm()
        self.set_parameters({"ADSB_TYPE": 1})  # mavlink
        self.set_parameter("AVD_F_ALT_MIN", 5)
        self.set_parameter("AVD_F_ACTION", 2)  # climb or descend
        here = self.mav.location()
        self.arm_vehicle()
        self.takeoff(10, mode='LOITER')  # takeoff in a stable mode
        self.change_mode("AVOID_ADSB")
        self.delay_sim_time(10)
        self.test_adsb_send_threatening_adsb_message(here, offset_ne=[10,0])
        self.delay_sim_time(10)
        self.land_and_disarm()

    def test_avoid_adsb_rcry_h(self):
       """Testing ADSB AVOID via RCY Move horizontal """
       self.progress("Starting ADSB AVOID via RCY test Horizontal")
       self.set_parameters({"AVD_ENABLE": 1})
       self.reboot_sitl()
       self.wait_ready_to_arm()
       self.set_parameters({"ADSB_TYPE": 1})  # mavlink
       self.set_parameter("AVD_F_ALT_MIN", 5)
       self.set_parameter("AVD_F_ACTION", 3)  # climb or descend
       here = self.mav.location()
       self.arm_vehicle()
       self.takeoff(10, mode='LOITER')  # takeoff in a stable mode
       self.change_mode("AVOID_ADSB")
    ...
    def test_avoid_adsb_rcry_r(self):
        """Testing ADSB AVOID via RCY REPORT only """
        self.progress("Starting ADSB AVOID via RCY test REPORT only")
        self.set_parameters({"AVD_ENABLE": 1})
        self.reboot_sitl()
        self.wait_ready_to_arm()
        self.set_parameters({"ADSB_TYPE": 1})  # mavlink
        self.set_parameter("AVD_F_ALT_MIN", 5)
        self.set_parameter("AVD_F_ACTION", 1)  # climb or descend
        here = self.mav.location()
        self.arm_vehicle()
        self.takeoff(10, mode='LOITER')  # takeoff in a stable mode
        self.change_mode("AVOID_ADSB")
    ...
    def test_avoid_adsb_rcry_per(self):
        """Testing ADSB AVOID via RCY perpendicular move """
        self.progress("Starting ADSB AVOID via RCY test perpendicular move")
        self.set_parameters({"AVD_ENABLE": 1})
        self.reboot_sitl()
        self.wait_ready_to_arm()
        self.set_parameters({"ADSB_TYPE": 1})  # mavlink
        self.set_parameter("AVD_F_ALT_MIN", 5)
        self.set_parameter("AVD_F_ACTION", 4)  # climb or descend
        here = self.mav.location()
        self.arm_vehicle()
        self.takeoff(10, mode='LOITER')  # takeoff in a stable mode
        self.change_mode("AVOID_ADSB")
    ...
    def test_avoid_adsb_rcry_RTL(self):
        """Testing ADSB AVOID via RCY RTL """
        self.progress("Starting ADSB AVOID via RCY test RTL")
        self.set_parameters({"AVD_ENABLE": 1})
        self.reboot_sitl()
        self.wait_ready_to_arm()
        self.set_parameters({"ADSB_TYPE": 1})  # mavlink
        self.set_parameter("AVD_F_ALT_MIN", 5)
        self.set_parameter("AVD_F_ACTION", 5)  # climb or descend
        here = self.mav.location()
        self.arm_vehicle()
        self.takeoff(10, mode='LOITER')  # takeoff in a stable mode
        self.change_mode("AVOID_ADSB")
    ...
    def test_avoid_adsb_rcry_Hover(self):
        """Testing ADSB AVOID via RCY HOVER """
        self.progress("Starting ADSB AVOID via RCY test HOVER")
        self.set_parameters({"AVD_ENABLE": 1})
        self.reboot_sitl()
        self.wait_ready_to_arm()
        self.set_parameters({"ADSB_TYPE": 1})  # mavlink
        self.set_parameter("AVD_F_ALT_MIN", 5)
        self.set_parameter("AVD_F_ACTION", 6)  # climb or descend
        here = self.mav.location()
        self.arm_vehicle()
        self.takeoff(10, mode='LOITER')  # takeoff in a stable mode
        self.change_mode("AVOID_ADSB")
    ...
    def test_mode_auto_mission(self):
        """Testing AUTO mode mission execution"""
        
        self.wait_ready_to_arm()
    
        self.progress("Loading mission")
        self.load_mission("TIHAN-MISSION.txt")
    
        self.arm_vehicle()
        self.set_mode("AUTO")
    
        self.progress("Waiting for mission completion")
        self.wait_disarmed(timeout=600)
    
        self.progress("AUTO mission completed")

    def test_solo_button_commands(self):
        """LCOV: cover MAV_CMD_SOLO_BTN_* handlers"""

        self.progress("=== SOLO BUTTON COMMAND TEST START ===")

        self.wait_ready_to_arm()

        # ---------------------------------------------------------------------
        # 1. FLY_CLICK (disarmed)
        # ---------------------------------------------------------------------
        self.progress("MAV_CMD_SOLO_BTN_FLY_CLICK")

        self.mav.mav.command_long_send(
            1, 1,
            mavutil.mavlink.MAV_CMD_SOLO_BTN_FLY_CLICK,
            0,
            0, 0, 0, 0, 0, 0, 0   # param1..param7 (ONLY 7!)
        )

        self.delay_sim_time(1)

        # ---------------------------------------------------------------------
        # 2. FLY_HOLD (disarmed → arm)
        # ---------------------------------------------------------------------
        self.progress("MAV_CMD_SOLO_BTN_FLY_HOLD (disarmed -> arm)")

        self.mav.mav.command_long_send(
            1, 1,
            mavutil.mavlink.MAV_CMD_SOLO_BTN_FLY_HOLD,
            0,
            5, 0, 0, 0, 0, 0, 0   # param1 = takeoff altitude
        )

        self.wait_armed()

        # ---------------------------------------------------------------------
        # 3. FLY_HOLD (landed + armed → takeoff)
        # ---------------------------------------------------------------------
        self.progress("MAV_CMD_SOLO_BTN_FLY_HOLD (landed -> takeoff)")
        ...
        self.wait_altitude(0,4, relative=True)

        # ---------------------------------------------------------------------
        # 4. FLY_HOLD (flying → land)
        # ---------------------------------------------------------------------
        self.progress("MAV_CMD_SOLO_BTN_FLY_HOLD (flying -> land)")
        ...
        # ---------------------------------------------------------------------
        # 5. PAUSE_CLICK (landed + armed → disarm)
        # ---------------------------------------------------------------------
        self.progress("MAV_CMD_SOLO_BTN_PAUSE_CLICK (landed -> disarm)")
        ...
        # 6. Re-arm + takeoff
        # ---------------------------------------------------------------------
        self.progress("Re-arming for PAUSE while flying test")

        self.arm_vehicle()
        self.takeoff(5)

        # ---------------------------------------------------------------------
        # 7. PAUSE_CLICK while flying (BRAKE / ALT_HOLD)
        # ---------------------------------------------------------------------
        self.progress("MAV_CMD_SOLO_BTN_PAUSE_CLICK (flying)")
        ...
        self.progress("=== SOLO BUTTON COMMAND TEST COMPLETE ===")
    ...
    #New test for brake timeout_to_loiter_ms
    def test_brake_timeout_to_loiter(self):
        """LCOV: cover ModeBrake::timeout_to_loiter_ms"""

        self.progress("=== BRAKE timeout_to_loiter_ms coverage test ===")
        ...
        # Ensure not in a guided/shot mode
        self.change_mode("LOITER")
        self.delay_sim_time(2)

        # ------------------------------------------------------------------
        # Trigger BRAKE via SOLO PAUSE button
        # This MUST call timeout_to_loiter_ms(2500)
        # ------------------------------------------------------------------
        self.progress("Triggering MAV_CMD_SOLO_BTN_PAUSE_CLICK (BRAKE)")
        ...
        # Give time for BRAKE mode and timeout setup
        self.delay_sim_time(1)

        # Optional: wait long enough to let timeout expire
        # (covers follow-up behavior, not required for this function)
        self.delay_sim_time(3)
        ...
        self.progress("=== BRAKE timeout coverage complete ===")

    #New test RTL SET SPEED
    def test_rtl_set_speed_functions(self):
        """LCOV: cover ModeRTL::set_speed_* functions"""

        self.progress("=== RTL set_speed_* coverage test ===")
        ...
        # -------------------------------------------------------------
        # 1. Horizontal speed (set_speed_NE_ms)
        # -------------------------------------------------------------
        self.progress("RTL: set horizontal speed")

        self.mav.mav.command_long_send(
            1, 1,
            mavutil.mavlink.MAV_CMD_DO_CHANGE_SPEED,
            0,
            1,      # param1: speed type = horizontal
            5.0,    # param2: speed (m/s)
            -1,     # param3: throttle (unused)
            0, 0, 0, 0
        )
        ...
        # -------------------------------------------------------------
        # 2. Climb speed (set_speed_up_ms)
        # -------------------------------------------------------------
        self.progress("RTL: set climb speed")

        self.mav.mav.command_long_send(
            1, 1,
            mavutil.mavlink.MAV_CMD_DO_CHANGE_SPEED,
            0,
            2,      # param1: speed type = climb
            2.0,    # param2: climb speed (m/s)
            -1,
            0, 0, 0, 0
        )
        ...
        # -------------------------------------------------------------
        # 3. Descent speed (set_speed_down_ms)
        # -------------------------------------------------------------
        self.progress("RTL: set descent speed")

        self.mav.mav.command_long_send(
            1, 1,
            mavutil.mavlink.MAV_CMD_DO_CHANGE_SPEED,
            0,
            3,      # param1: speed type = descent
            1.5,    # param2: descent speed (m/s)
            -1,
            0, 0, 0, 0
        )
        ...
        self.progress("=== RTL set_speed_* coverage complete ===")


    #new LAND NO USE GPS test
    def test_land_do_not_use_gps(self):
        """LCOV: cover ModeLand::do_not_use_GPS via GPS failsafe"""

        self.progress("=== LAND do_not_use_GPS coverage test ===")
        # -------------------------------------------------
        # Arm and takeoff (with GPS)
        # -------------------------------------------------
        self.wait_ready_to_arm()
        self.arm_vehicle()
        self.takeoff(5)
        self.set_parameter("RC7_OPTION", 65)  # Enable GPS failsafe

        # -------------------------------------------------
        # Enter LAND mode (GPS-based landing)
        # -------------------------------------------------
        self.set_rc(7, 2000)  # Ensure GPS failsafe is not triggered yet
        self.change_mode("LAND")
        self.delay_sim_time(1)

        # -------------------------------------------------
        # Trigger GPS failsafe
        # This forces ModeLand::do_not_use_GPS()
        # -------------------------------------------------
        self.progress("Triggering GPS failsafe")

        # Disable GPS in SITL
        # Give time for failsafe handling
        self.delay_sim_time(2)
        ...
        self.wait_disarmed()
        self.set_rc(7, 1000)  # Restore GPS failsafe

        self.progress("=== LAND do_not_use_GPS coverage complete ===")

    #New code Guided wp controle
    def test_guided_wpnav_control(self):
        """LCOV: cover ModeGuided WP navigation (wp_control_start/run)"""

        self.progress("=== Guided WPNav control coverage test ===")

        # -------------------------------------------------
        # Enable WPNav for Guided
        # -------------------------------------------------
        self.set_parameter("GUID_OPTIONS", 64)  # WPNavUsedForPosControl
        ...
        self.change_mode("GUIDED")
        self.delay_sim_time(1)
        self.progress("GUIDED: set horizontal speed")
        ...
        # 2. Climb speed (set_speed_up_ms)
        # -------------------------------------------------------------
        self.progress("GUIDED: set climb speed")
        ...
        # 3. Descent speed (set_speed_down_ms)
        # -------------------------------------------------------------
        self.progress("GUIDED: set descent speed")
        ...
        self.delay_sim_time(1)
        # -------------------------------------------------
        # Send a GUIDED waypoint (forces WP submode)
        # -------------------------------------------------
        self.progress("Sending guided waypoint")

        home= self.mav.location()
        target_lat = home.lat + 0.00001
        target_lon = home.lng + 0.00001
        ...
        self.progress("=== Guided WPNav coverage complete ===")

    #new set pos vel target guided
    def test_guided_set_pos_vel(self):
        """LCOV: cover ModeGuided::set_pos_vel_NEU_m"""

        self.progress("=== Guided set_pos_vel_NEU_m coverage test ===")
        ...
        # -------------------------------------------------
        # Send position + velocity target (LOCAL_NED)
        # -------------------------------------------------
        self.progress("Sending SET_POSITION_TARGET_LOCAL_NED (pos + vel)")
        ...
        self.mav.mav.set_position_target_local_ned_send(
            ...
            mavutil.mavlink.MAV_FRAME_LOCAL_NED,
            type_mask,
            2.0, 2.0, -5.0,         # position (x,y,z) meters
            0.5, 0.0, 0.0,          # velocity (vx,vy,vz) m/s
            0, 0, 0,                # acceleration (ignored)
            0, 0                    # yaw, yaw_rate (ignored)
        )

        # Let guided controller run
        self.delay_sim_time(2)
        ...
        self.progress("=== set_pos_vel_NEU_m coverage complete ===")
    ...
    def testExternalControlCoverage(self):
        ...
        # Use a small offset in scaled units (degrees * 1e7)
        small_offset = int(0.00001 * 1e7)   # ~1e-5 degree
        target_lat = start_lat + small_offset
        target_lon = start_lon + small_offset
        target_alt = 10

        # Send SET_POSITION_TARGET_GLOBAL_INT (should be rejected, not in GUIDED)
        self.mav.mav.set_position_target_global_int_send(
            int(self.get_sim_time() * 1e6),           # time_boot_ms
            self.mav.target_system,
            self.mav.target_component,
        ...
        # verify vehicle did NOT move toward target (non-guided should ignore)
        loc_after = self.get_global_position_int()
        dist_before = abs(start_lat - target_lat) + abs(start_lon - target_lon)
        dist_after  = abs(loc_after.lat - target_lat) + abs(loc_after.lon - target_lon)
        # In non-guided case, distance should not decrease
        self.assert_greater_equal(dist_after, dist_before, "Vehicle moved toward target while not GUIDED")
        ...
        self.wait_ready_to_arm()
        self.arm_vehicle()
        self.wait_armed(timeout=5)

        # takeoff to a safe altitude so GUIDED accepts position targets
        self.takeoff(10, mode="GUIDED")
        ...
        #loc = self.get_global_position_int()
        # Send new target
        # Send new target (small offset so vehicle will move noticeably quickly)
        target_lat = start_lat + small_offset
        target_lon = start_lon + small_offset
        target_alt = 15
        ...
        # allow time for guided control to act
        self.delay_sim_time(4)
        self.progress("Sent target in GUIDED mode, should trigger set_target_location()")

        # assert we moved toward target
        new_loc = self.get_global_position_int()
        before_dist = abs(start_lat - target_lat) + abs(start_lon - target_lon)
        after_dist  = abs(new_loc.lat - target_lat) + abs(new_loc.lon - target_lon)
        self.assert_less(after_dist, before_dist, "Vehicle did not progress toward target in GUIDED mode")
        ...
        self.wait_landed_and_disarmed(min_alt=3,timeout=20)
        
        self.progress("External Control Coverage Test complete ")


    def test_invalid_initial_mode_fallback(self):
        self.set_parameter("INITIAL_MODE", 99)  # invalid mode
        self.wait_ready_to_arm()
        self.change_mode("STABILIZE")

    def test_dead_reckoning_relative_position(self):
        self.set_parameter("EK3_ENABLE", 1)
        self.set_parameter("EK3_DR_ENABLE", 1)
        self.set_parameter("FLOW_TYPE", 0)   # disable optical flow
        self.delay_sim_time(5)

    def test_relative_position_const_pos_reject(self):
        self.arm_vehicle()
        self.set_parameter("EK3_SRC1_POSXY", 0)  # no rel pos source
        self.delay_sim_time(3) 

    def test_ekf_altitude_velocity_fail(self):
        self.set_parameter("EK3_SRC1_VELZ", 0)
        self.delay_sim_time(3)    

    def test_tradheli_auto_armed(self):
        self.set_parameter("FRAME_CLASS", 6)  # HELI
        self.wait_ready_to_arm()
        self.arm_vehicle()
        self.set_rc(3, 1600)
        self.delay_sim_time(2)

    def test_tri_motor_allocation(self):
        self.set_parameter("FRAME_CLASS", 1)  # TRI
        self.reboot_sitl()      

    def test_single_motor_frame(self):
        self.set_parameter("FRAME_CLASS", 2)  # SINGLE
        self.reboot_sitl()    

    def test_coax_motor_frame(self):
        self.set_parameter("FRAME_CLASS", 3)  # COAX
        self.reboot_sitl()     

    def test_tailsitter_motor_frame(self):
        self.set_parameter("FRAME_CLASS", 10)
        self.reboot_sitl()    

    def test_scripting_motor_frame(self):
        self.set_parameter("SCR_ENABLE", 1)
        self.set_parameter("FRAME_CLASS", 12)
        self.reboot_sitl()    

    def test_y6_pid_defaults(self):
        self.set_parameter("FRAME_CLASS", 4)  # Y6
        self.reboot_sitl()   

    def test_brushed_motor_defaults(self):
        self.set_parameter("MOT_PWM_TYPE", 1)  # brushed
        self.reboot_sitl()   

    def test_drift_braker_reset(self):
        self.change_mode("DRIFT")
        self.arm_vehicle()

        # give pitch input (non-zero)
        self.set_rc(2, 1600)
        self.delay_sim_time(2)

    def test_drift_throttle_unlimited(self):
        self.change_mode("DRIFT")
        self.arm_vehicle()

        self.set_rc(3, 1600)   # throttle > zero
        self.delay_sim_time(2)

    def test_drift_ground_idle(self):
        self.change_mode("DRIFT")
        self.arm_vehicle()

        self.set_rc(3, 1000)   # throttle zero
        self.delay_sim_time(2)   

    def test_drift_throttle_assist(self):
        self.change_mode("DRIFT")
        self.arm_vehicle()

        # throttle between 0.213 and 0.787
        self.set_rc(3, 1400)
        self.delay_sim_time(2) 

    def test_guided_api_reject_not_guided(self):
        self.change_mode("STABILIZE")

        loc = self.mav.location()
        self.assertFalse(self.mav.set_target_location(loc))
        self.assertFalse(self.mav.start_takeoff(5))

    def test_guided_api_success(self):
        self.change_mode("GUIDED")
        self.arm_vehicle()

        loc = self.mav.location()
        self.assertTrue(self.mav.set_target_location(loc))
        self.assertTrue(self.mav.start_takeoff(3))

        self.mav.set_target_velocity_ned(1, 0, 0)

    def test_guided_angle_and_rate(self):
        self.change_mode("GUIDED")
        self.arm_vehicle()

        self.mav.set_target_angle_and_climbrate(5, 0, 0, 0.5)
        self.mav.set_target_rate_and_throttle(5, 0, 0, 0.5)

    def test_register_custom_guided_mode(self):
        self.mav.register_custom_mode(100, "MYMODE", "MYMD")  

    def test_circle_mode_api(self):
        self.change_mode("CIRCLE")

        r = self.mav.get_circle_radius()
        self.mav.set_circle_rate(30)         
  
    def test_simple_mode_rotation(self):
        self.set_parameter("SIMPLE", 1)
        self.change_mode("STABILIZE")
    ...
    def test_super_simple_bearing(self):
        self.set_parameter("SIMPLE", 2)
        self.change_mode("STABILIZE")
        self.delay_sim_time(2)   
      
    def test_get_rate_ef_targets(self):
        self.arm_vehicle()
        self.takeoff(3)

        rates = self.mav.get_rate_ef_targets() 


    def test_motor_test_throttle_percent(self):
        self.wait_ready_to_arm()

        # MAV_CMD_DO_MOTOR_TEST
        self.mav.do_motor_test(
            motor=1,
            throttle_type=0,   # MOTOR_TEST_THROTTLE_PERCENT
            throttle=10,
            timeout=2,
            motor_count=1
        )
    ...
    def test_motor_test_pilot_throttle(self):
        self.wait_ready_to_arm()
        ...
        self.mav.do_motor_test(
            motor=1,
            throttle_type=2,   # MOTOR_TEST_THROTTLE_PILOT
            throttle=0,
            timeout=2,
            motor_count=1
        )
    ...
    def test_motor_test_compass_cal(self):
        self.wait_ready_to_arm()

        self.mav.do_motor_test(
            motor=1,
            throttle_type=3,   # MOTOR_TEST_COMPASS_CAL
            throttle=15,
            timeout=2,
            motor_count=1
        )
    ...
    def test_motor_test_rejection_paths(self):
        # force not initialised
        self.context_set("skip_arming_checks", True)

        # try motor test while not ready
        self.mav.do_motor_test(
            motor=1,
            throttle_type=0,
            throttle=10,
            timeout=1,
            motor_count=0   # triggers line 149
        )  

    def test_acro_mode_max_coverage_single(self):
        # --- Parameters to force branches ---
        self.set_parameter("ACRO_OPTIONS", 1 << 0)   # AIR_MODE
        self.set_parameter("ACRO_OPTIONS", self.get_parameter("ACRO_OPTIONS") | (1 << 1))  # RATE_LOOP_ONLY
        self.set_parameter("ACRO_TRAINER", 1)  # LIMITED
        self.set_parameter("ACRO_RP_RATE", 360)
        self.set_parameter("ACRO_Y_RATE", 360)
        self.set_parameter("ACRO_THR_MID", 0.6)
        ...
        # --- Force THROTTLE_UNLIMITED ---
        self.set_rc(3, 1600)  # throttle > zero

        # --- Aggressive stick input to exceed circular limit ---
        self.set_rc(1, 2000)  # roll
        self.set_rc(2, 2000)  # pitch
        self.set_rc(4, 1800)  # yaw

        # --- Let attitude settle and exceed lean angle ---
        self.delay_sim_time(3)

        # --- Reduce throttle to trigger exit paths ---
        self.set_rc(3, 1000)
        self.delay_sim_time(1)
    ...
    def test_circle_mode_max_coverage_single(self):
        # --- Ensure RC input is valid ---
        self.wait_ready_to_arm()
        ...
        # --- Let circle initialise ---
        self.delay_sim_time(1)

        # -------- Radius change (covers line 67) --------
        # Pitch stick forward to change radius
        self.set_rc(2, 1600)   # pitch up
        self.delay_sim_time(2)

        # -------- Speed changing logic (covers 83–87, 95–97, 100–101) --------
        # Roll stick to initiate rate change
        self.set_rc(1, 1700)   # roll right
        self.delay_sim_time(2)

        # -------- Trigger zero-rate transition --------
        # Release roll stick to hit zero-rate logic
        self.set_rc(1, 1500)
        self.delay_sim_time(1)
    ...
    def test_gcs_log_radio_bit(self):
        # Ensure logging is enabled
        self.set_parameter("LOG_BITMASK", 0xFFFFFFFF)

        # Wait for GCS connection to be active
        self.wait_gcs_heartbeat()

        # Force sending of radio status messages
        # This causes GCS_MAVLINK_Copter::log_radio_bit() to be queried
        self.mav.request_data_stream_send(
            target_system=self.mav.target_system,
            target_component=self.mav.target_component,
            stream_id=10,   # MAV_DATA_STREAM_RAW_SENSORS (radio-related)
            message_rate=1,
            on_off=1
        )

        # Give SITL time to process logging path
        self.delay_sim_time(2)

    def test_fence_breach_rtl_path(self):
        # --- Enable fence ---
        self.set_parameter("FENCE_ENABLE", 1)
        self.set_parameter("FENCE_TYPE", 2)     # ALT fence
        self.set_parameter("FENCE_ALT_MAX", 10)
        self.set_parameter("FENCE_ACTION", 1)   # RTL_AND_LAND

        # --- Arm and take off ---
        self.wait_ready_to_arm()
        self.arm_vehicle()
        self.takeoff(15)   # exceed fence altitude

        # --- Allow async fence detection ---
        self.delay_sim_time(3)

        # Fence should trigger RTL
        self.wait_mode("RTL")    


    def test_rtl_basic_flow(self):
        self.wait_ready_to_arm()
        self.change_mode("LOITER")
        self.arm_vehicle()
        self.takeoff(10)
        ...
        # allow RTL to complete
        self.delay_sim_time(60)

        self.wait_disarmed()


    def test_guided_position(self):
        self.wait_ready_to_arm()
        self.change_mode("GUIDED")
        self.arm_vehicle()
        ...
        self.set_guided_position(5, 0, -10)
        self.delay_sim_time(5)


    def test_guided_velocity(self):
        self.wait_ready_to_arm()
        self.change_mode("GUIDED")
        self.arm_vehicle()
        self.takeoff(10)

        # SET_POSITION_TARGET_LOCAL_NED (velocity)
        self.set_guided_velocity(1, 0, 0)
        self.delay_sim_time(5)

    def test_guided_angle(self):
        self.wait_ready_to_arm()
        self.change_mode("GUIDED")
        self.arm_vehicle()
        self.takeoff(5)
        ...
        # SET_ATTITUDE_TARGET
        self.set_attitude_target(roll=0.1, pitch=0.0, yaw=0.0, thrust=0.6)
        self.delay_sim_time(5)

    def test_guided_pause_resume(self):
        self.wait_ready_to_arm()
        self.change_mode("GUIDED")
        self.arm_vehicle()
        self.takeoff(5)
    ...
    def testRCTransmitterTuningCoverage(self):
        """Force execution of Copter::tuning() switch cases"""

        self.progress("Starting RC Transmitter Tuning Coverage Test")

        # --- Setup ---
        self.set_parameter("RC_TUNING_CHAN", 7)     # knob channel
        self.set_parameter("RC7_OPTION", 55)        # TRANSMITTER_TUNING
        self.set_parameter("RC_TUNING_MIN", -1)
        self.set_parameter("RC_TUNING_MAX", 1)
        ...
        tuning_params = [
            3,   # TUNING_RATE_ROLL_PITCH_KP
            4,   # TUNING_RATE_ROLL_PITCH_KI
            5,   # TUNING_RATE_ROLL_PITCH_KD
            12,  # TUNING_ALTITUDE_HOLD_KP
            15,  # TUNING_ACCEL_Z_KP
            16,  # TUNING_ACCEL_Z_KI
            17,  # TUNING_ACCEL_Z_KD
            28,  # TUNING_RATE_YAW_FILT
            29,  # TUNING_RATE_MOT_YAW_HEADROOM
        ]

        for param in tuning_params:
            self.set_parameter("RC_TUNING_PARAM", param)
            move_knob(1100)
            move_knob(1500)
            move_knob(1900)
        ...
        # ---------------- ACRO tuning ----------------
        self.change_mode("ACRO")
        self.set_parameter("RC_TUNING_PARAM", 20)  # TUNING_ACRO_RP_RATE
        move_knob(1800)

        self.set_parameter("RC_TUNING_PARAM", 21)  # TUNING_ACRO_YAW_RATE
        move_knob(1800)

        # ---------------- CIRCLE tuning ----------------
        self.change_mode("CIRCLE")
        self.set_parameter("RC_TUNING_PARAM", 26)  # TUNING_CIRCLE_RATE
        move_knob(1700)

        # ---------------- DECLINATION ----------------
        self.set_parameter("RC_TUNING_PARAM", 25)  # TUNING_DECLINATION
        move_knob(1600)
        ...
        self.progress("RC Transmitter Tuning Coverage Test complete")

    def ModeFlowHold(self):
        """Coverage-driven test for FLOWHOLD mode"""

        FLOWHOLD = 20  # Copter mode number

        # ---------- Enable Optical Flow ----------
        self.set_parameter("FLOW_TYPE", 10)        # SITL optical flow
        self.set_parameter("FLOW_ENABLE", 1)
        self.set_parameter("FLOW_POS_X", 0)
        self.set_parameter("FLOW_POS_Y", 0)
        self.set_parameter("FLOW_POS_Z", 0)
        self.set_parameter("FLOW_ORIENT_YAW", 0)

        # FlowHold parameters
        self.set_parameter("FHLD_FLOW_MAX", 2.0)
        self.set_parameter("FHLD_FILT_HZ", 10)
        self.set_parameter("FHLD_QUAL_MIN", 1)
        self.set_parameter("FHLD_BRAKE_RATE", 15)

        # ---------- Arm & Takeoff ----------
        self.change_mode("ALT_HOLD")
        self.arm_vehicle()
        self.takeoff(alt_min=3, mode="ALT_HOLD")
        self.wait_altitude(3, 5, relative=True)
        self.delay_sim_time(3)
        ...
        self.progress("Generate optical flow via lateral movement")
        self.set_rc(1, 1600)   # roll
        self.set_rc(2, 1400)   # pitch
        self.delay_sim_time(2)

        # ---------- Stick release → braking ----------
        self.set_rc(1, 1500)
        self.set_rc(2, 1500)
        self.delay_sim_time(4)

        # ---------- Force integrator path ----------
        for _ in range(3):
            self.set_rc(1, 1550)
            self.delay_sim_time(0.3)
            self.set_rc(1, 1500)
            self.delay_sim_time(0.6)
        ...
        # ---------- Low quality → AltHold fallback ----------
        self.progress("Drop flow quality")
        self.set_parameter("FHLD_QUAL_MIN", 200)
        self.delay_sim_time(2)
        ...
        # ---------- Height estimator paths ----------
        self.progress("Trigger height estimator")
        self.set_rc(3, 1550)
        self.delay_sim_time(2)
        self.set_rc(3, 1450)
        self.delay_sim_time(2)
        ...
        # ---------- Re-arm → Flying ----------
        self.arm_vehicle()
        self.takeoff(alt_min=3, mode="FLOWHOLD")
        self.delay_sim_time(3)
...
#        self.takeoffAndMoveAway()
    ...
    def test_esc_calibration_guard(self):
        """ESC calibration startup guard paths (SITL-safe)."""

        self.start_subtest("ESC calibration does not trigger in SITL")

        # Explicitly disable ESC calibration
        self.set_parameter("ESC_CALIBRATION", 0)

        # Ensure throttle is LOW
        self.set_rc(3, 1000)

        # Reboot to force esc_calibration_startup_check()
        self.reboot_sitl()

        # If ESC calibration triggered, SITL would hang here
        self.wait_ready_to_arm()

        self.progress("ESC calibration guard path executed successfully")

    def ModeTurtle(self):
        """ModeTurtle guard-path coverage (SITL safe)"""

        self.start_subtest("ModeTurtle init rejection paths")

        # Ensure disarmed
        self.disarm_vehicle()

        # Ensure sticks are NOT centered → should reject
        self.set_rc(1, 1600)  # roll
        self.set_rc(2, 1500)
        self.set_rc(3, 1000)
        self.set_rc(4, 1500)

        # Attempt to enter Turtle mode
        TURTLE = 26  # Copter mode number for Turtle
        self.change_mode(TURTLE)

        # Should immediately fall back (init returns false)
        self.delay_sim_time(1)
        self.progress("Turtle rejected with non-centered sticks")
        ...
        # Try again – still should reject due to no DSHOT
        self.change_mode(TURTLE)
        self.delay_sim_time(1)
        self.progress("Turtle rejected due to missing DSHOT (expected in SITL)")

        # Exit path coverage
        self.change_mode('STABILIZE')
        self.progress("Exited Turtle mode cleanly")
    ...
    def ModeZigZag(self):
        ...
        self.set_parameter("ZIGZAG_AUTO_ENABLE", 1)
        self.set_parameter("ZIGZAG_SIDE_DIST", 6.0)
        self.set_parameter("ZIGZAG_LINE_NUM", 3)
        self.set_parameter("ZIGZAG_WP_DELAY", 1)
        self.set_parameter("LOIT_OPTIONS", 0)

        self.takeoff(alt_min=5, mode='LOITER')
        self.wait_altitude(5, 10)
        self.delay_sim_time(2)

        ZIGZAG = 24
        slowdown_speed = 0.3

        self.start_subtest("ZigZag AUTO coverage test")

        for j in range(4):
            self.progress("## Heading alignment j=%d ##" % j)
            ...
            # ==========================================================
            # ### NEW: ENTER AUTO STATE (this unlocks 60% coverage)
            # ==========================================================
            self.start_subtest("## Trigger ZigZag AUTO ##")
            self.set_rc(8, 1100)          # AUTO start
            self.delay_sim_time(1.5)
            ...
            # AUTO A->B and B->A
            # ----------------------------------------------------------
            self.start_subtest("AUTO A->B")
            self.set_rc(8, 1900)
            self.wait_distance(15)
            self.delay_sim_time(1)

            self.start_subtest("AUTO B->A")
            self.set_rc(8, 1100)
            self.wait_distance(15)
            self.delay_sim_time(1)

            # ==========================================================
            # ### NEW: force reached_destination + WP_DELAY
            # ==========================================================
            self.delay_sim_time(2.0)

            # ==========================================================
            # ### NEW: suspend auto (manual override)
            # ==========================================================
            self.start_subtest("Suspend AUTO via manual input")
            self.set_rc(2, 1300)
            ...
            # ### NEW: resume AUTO
            # ==========================================================
            self.start_subtest("Resume AUTO")
            self.set_rc(8, 1100)
            self.wait_distance(8)
            self.delay_sim_time(1)

            # ==========================================================
            # ### NEW: abort AUTO via mode switch
            # ==========================================================
            self.start_subtest("Abort AUTO via mode change")
            self.change_mode("LOITER")
            self.delay_sim_time(1)
...
#        self.delay_sim_time(1)
    ...
    def test_log_radio_bit(self):
        """Test to trigger GCS_MAVLINK_Copter::log_radio_bit()"""

        self.progress("Starting log_radio_bit coverage test")

        # Ensure vehicle is ready
        self.wait_ready_to_arm()

        # Request extended status stream (this invokes log_radio_bit on GCS side)
        ...
        # Allow SITL time to process
        self.delay_sim_time(2)

        self.progress("Completed log_radio_bit test")
    ...
    def tests1a(self):
        ...
        ret.extend([
             ...
             self.test_log_radio_bit,
             self.test_esc_calibration_guard,
             self.testRCTransmitterTuningCoverage,
             self.ModeTurtle,
             self.test_invalid_initial_mode_fallback,
             self.test_dead_reckoning_relative_position,
             self.test_relative_position_const_pos_reject,
             self.test_ekf_altitude_velocity_fail,
             self.test_tradheli_auto_armed,
             self.test_tri_motor_allocation,
             self.test_single_motor_frame,
             self.test_coax_motor_frame,
             self.test_tailsitter_motor_frame,
             self.test_scripting_motor_frame,
             self.test_y6_pid_defaults,
             self.test_brushed_motor_defaults,
             self.test_drift_braker_reset,
             self.test_drift_throttle_unlimited,
             self.test_drift_ground_idle,
             self.test_drift_throttle_assist,
             self.test_guided_api_reject_not_guided,
             self.test_guided_api_success,
             self.test_guided_angle_and_rate,
             self.test_register_custom_guided_mode,
             self.test_circle_mode_api,
             self.test_simple_mode_rotation,
             self.test_super_simple_bearing,
             self.test_get_rate_ef_targets,
             self.test_motor_test_throttle_percent,
             self.test_motor_test_pilot_throttle,
             self.test_motor_test_compass_cal,
             self.test_motor_test_rejection_paths,
             self.test_acro_mode_max_coverage_single,
             self.test_circle_mode_max_coverage_single,
             self.test_gcs_log_radio_bit,
             self.test_fence_breach_rtl_path,
             self.test_rtl_basic_flow,
        ])
        return ret
    ...
    def tests1b(self):
        ...
        ret = ([
             ...
             self.test_guided_position,
             self.test_guided_velocity,
             self.test_guided_angle,
             self.test_guided_pause_resume,
        ])
        return ret
    ...
    def tests2a(self):
        ...
        ret = ([
            ...
            # self.testScripting6,
            # self.testScripting7,
            # self.testScripting_ADSB,
            # self.testFlowholdCoverage,
            # self.testGuidedNoGPS,
            # self.testScripting8
            #self.testScripting9
            # self.test_avoid_adsb_rcry_cd,
            # self.test_avoid_adsb_rcry_h,
            # self.test_avoid_adsb_rcry_r,
            # self.test_avoid_adsb_rcry_RTL,
            # self.test_avoid_adsb_rcry_Hover,
            # self.test_avoid_adsb_rcry_per,
            #self.test_solo_button_commands,
            #self.test_brake_timeout_to_loiter,
            #self.test_rtl_set_speed_functions,
            #self.test_land_do_not_use_gps,
            #self.test_guided_wpnav_control,
            self.test_guided_set_pos_vel
        ])
        return ret
