...
#        self.takeoffAndMoveAway()
    ...
    def test_esc_calibration_coverage(self):
        """Trigger ESC-calibration code paths in SITL (safe; do NOT run on real craft)."""
        from pymavlink import mavutil
        self.progress("Starting ESC calibration coverage test (SITL only)")

        self.wait_ready_to_arm()

        # Request preflight calibration — this triggers ESC calibration handling in firmware
        # NOTE: DO NOT run on a real vehicle without safety precautions.
        # param1..7 vary by firmware; sending generic command triggers preflight calibration handler.
        self.mav.mav.command_long_send(
            ...
            0,   # confirmation
            0,   # param1: gyro (0/no)
            1,   # param2: magnetometer (example) -- firmware may interpret differently
            0, 0, 0, 0, 0
        )

        # allow time for SITL to run through the calibration flow
        self.delay_sim_time(3)

        # Some ESC cal modes also require a small passthrough: request another command that
        # typically toggles ESC passthrough path (only in SITL)
        try:
            self.mav.mav.command_long_send(
                self.mav.target_system,
                self.mav.target_component,
        ...
        self.delay_sim_time(2)
        self.progress("ESC calibration coverage test finished (SITL)")       

    def ModeZigZag(self):
        '''test zigzag mode'''
        # set channel 8 for zigzag savewp and recentre it
        self.set_parameter("RC8_OPTION", 61)
        # enable zigzag auto in case it is not enabled by default
        # use the exact parameter name from the source (grep AP_GROUPINFO("AUTO_ENABLE"))
        self.set_parameter("ZIGZAG_AUTO_ENABLE", 1)   # <--- replace with the exact param name if different
        # make sure side distance, line number and wp delay are friendly for the test:
        self.set_parameter("ZIGZAG_SIDE_DIST", 6.0)   # shorter sideways distance so tests complete faster
        self.set_parameter("ZIGZAG_LINE_NUM", 6)      # number of lines to allow looping
        self.set_parameter("ZIGZAG_WP_DELAY", 1)      # smaller delay at waypoints
        self.set_parameter("LOIT_OPTIONS", 0)

        self.takeoff(alt_min=5, mode='LOITER')
        # allow controllers to initialise and become active
        self.wait_altitude(5, 10)          # ensure we reached altitude and controllers active
        self.delay_sim_time(1.5)           # extra time for internal state setup
        ...
        while j < 4:
            ...
            # robustly store point A (two toggles + mid)
            for _ in range(2):
                self.set_rc(8, 1100)  # record point A
                self.delay_sim_time(0.45)
            self.set_rc(8, 1500)
            self.delay_sim_time(0.5)   
...
#        self.delay_sim_time(1)
    ...
    def tests1a(self):
        ...
        ret.extend([
             ...
             self.test_esc_calibration_coverage,
        ])
        return ret
